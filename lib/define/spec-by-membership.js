import Spec from "@/lib/core/spec.js";
import {invalid_symbol} from "@/lib/core/misc/symbols.js";
import registry from "@/lib/core/registry-of-specs/registry.js";
import ProblemTrace from "@/lib/core/problem-trace.js";
import Problem from "@/lib/core/problem.js";
 ...
 * Structure of the semantics is not defined here!
 * 
 * @see "@/README.md"
 * @see "@/lib/_info.md"
 * @see "@/test/feat/map-entity.test.js"
 */
export function $map(...maybeNsKeys) {
    ...
    return new Spec({
        subspecConfig: [
            mapEntity,
            objMap => ({
                subdata: Object.keys(objMap),
                subspec: arrayContaining(nskeys),
            }),
            objMap => semantic(objMap),
        ]
    })
}
...
function semantic(obj) {
    return Object.keys(objMap)
                ...
                .map(nskey => ({
                    subspec: registry.get(nskey),
                    subdata: objMap[nskey]
                }))
}
...
function arrayContaining(obligatoryValues) {
    return new Spec({
        subspecConfig: 
            obligatoryValues.map(
                v => new Spec(`contains "${v}"?`, array => array.includes(v))
            )
    })
}
...
export function $coll({spec, count, minCount, maxCount, distinct, kind, into}) {
    const properSpec = toSpec(spec)

    return new Spec({
        type: "collection",
        conform: collConformFactory(into),
        children: {spec: properSpec, count, minCount, maxCount, distinct, kind},
        predicateStringTemplate: children => `collection[${properSpec}]`,
        problemFinders: [
            checkIfCollection,
            checkIfCorrectCount,
...
function checkIfCollection(data, collSpec) {
    const collection = collConformFactory(Array)(data)
...
function checkIfCorrectKind(data, collSpec) {
    ...
    if(!collSpec.children.kind || 
        collSpec.children.kind(collection))
        return new ProblemTrace([])
    else
        return new ProblemTrace({
            invalidValue: data,
